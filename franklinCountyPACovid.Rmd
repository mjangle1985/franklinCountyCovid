...
daySinceFirstCase <- 40

franklinCountyCorona <- data.frame("day" = c(seq(1,daySinceFirstCase)), "dates" = seq(as.Date("2020-03-20"), by = "days", length.out = daySinceFirstCase), "cases" = c(1, 1, 1, 1, 3, 5, 5, 5, 7, 11, 12, 19, 21, 23, 26, 30, 32, 39, 43, 52, 57, 64, 69, 78, 80, 89, 106, 111, 115, 143, 152, 151, 164, 181, 205, 227, 237, 264, 284, 313))
...
```{r use}
newDay <-data.frame("day" = c(seq(1, 40)))
nextTwentyDays <-predict(fit, newDay)
nextTwentyDays <- as.data.frame(nextTwentyDays)
tmp <- seq(as.Date("2020-03-20"), by = "days", length.out = 40)
...
ggplot(finalModel, aes(x = dates)) +
  ...
  ggtitle("Franklin County PA Confirmed Covid19 Cases Model Days") +
  theme_bw()
```